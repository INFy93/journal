<template>
    <div v-if="device.node">
        <header-title>
            {{ isEmpty ? 'Добавить' : 'Обновить' }} оборудование на устройство {{ device.device_name }} ({{
    ...
    <div v-if="device.node" class="w-full px-3 mt-2 mb-6 md:w-2/3 md:mb-0">
        <label for="time" class="block text-sm font-medium text-gray-700">Устройство</label>
        <textarea v-model="stuffInfo.stuff" id="message" rows="4"
    ...
    <div class="flex mt-2 ml-3 space-x-2">
        <button @click="saveStuff(stuff.id)" type="submit"
