...
export default function useNodes() {
    ...
    const getNodes = async (id) => {
        let response = await axios.get("/api/v1/node/" + id);
    ...
    const getSingleNode = async (id) => {
        node.value = [];
        let response = await axios.get("/api/v1/nodes/" + id);
    ...
    const storeNodes = async (data) => {
        nodes_errors.value = "";
        try {
            await axios.post("/api/v1/nodes", data);
    ...
    const updateNode = async (id) => {
        nodes_errors.value = "";
        try {
            await axios.put("/api/v1/nodes/" + id, node.value);
